    ...
    <main className="min-h-screen w-full bg-[#0a0a0a] px-4 py-10 text-slate-100 sm:px-8 lg:px-16">
      <div className="mx-auto flex max-w-6xl flex-col gap-8">
        <header className="space-y-3 border-b border-slate-800 pb-4">
          ...
          <h1 className="text-2xl font-semibold text-slate-50">All Projects</h1>
          <p className="max-w-xl text-sm text-slate-400">
            A comprehensive view of my cybersecurity, tooling, and application projects, with
          ...
          {filtered.map((project) => (
            ...
            >
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-[10px] font-mono text-slate-400">
                {project.tech.map((t) => (
                  <span key={t} className="rounded-full bg-slate-900/90 px-2 py-0.5">
                    {t}
                  </span>
                ))}
              </div>
